...
RUN --mount=type=cache,target=/build \
	...
	wget --no-hsts --quiet \
		"https://nodejs.org/download/release/v24.7.0/node-v24.7.0-linux-$ARCH.tar.xz" \
		"https://nodejs.org/download/release/v24.7.0/SHASUMS256.txt" \
		"https://nodejs.org/download/release/v24.7.0/SHASUMS256.txt.sig" && \
	sha256sum --quiet --check --strict --ignore-missing SHASUMS256.txt && \
	gpg --verify SHASUMS256.txt.sig SHASUMS256.txt 2>/dev/null && \
	tar --xz --extract --file="node-v24.7.0-linux-$ARCH.tar.xz" --exclude=bin/npx --exclude=bin/corepack --exclude=lib/node_modules/corepack --exclude=include --exclude=share --no-same-owner && \
	mv "node-v24.7.0-linux-$ARCH" /opt/node
